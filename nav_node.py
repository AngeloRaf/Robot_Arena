...
class NavNode(Node):
    ...
    def _state_retour_depart(self):
        ...
        if self._retour_phase == 'ADVANCE':
            ...
            target_dist = max(self.distance_forward * 0.85 + 0.05, 0.05)
    ...
    def _state_chercher_goal(self):
        ...
        Une fois centré → POUSSER_DROIT vers le goal correspondant.
        """
        target_goal = self.COLOR_TO_GOAL.get(self.push_color or self.target_color, -1)
        goal_marker = next((m for m in self.aruco_markers if m['id'] == target_goal), None)
